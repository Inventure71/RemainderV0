...
export function renderModelChatSidebar(container, context = {}) {
    container.innerHTML = `
        <style scoped>
            .model-chat-sidebar { width: 500px; max-width: 100vw; background: #262a34; border-left: 1px solid #363b47; display: flex; flex-direction: column; height: 100%; }
            ...
            .message-item { margin-bottom: 0.6em; padding: 0.5em 1em 0.5em 0.7em; border-radius: 5px; word-wrap: break-word; }
            ...
            .model-chat-input-row { display: flex; gap: 8px; align-items: center; margin-top: 0.7em; }
            .model-chat-input-row input { flex: 1; padding: 0.5em 0.8em; border-radius: 4px; border: 1px solid #44495a; background: #23272e; color: #fff; }
            .model-chat-input-row button { background: #3578e5; color: #fff; border: none; border-radius: 4px; padding: 0.5em 1.2em; font-weight: bold; cursor: pointer; }
        ...
        <div class="model-chat-sidebar">
            <div class="model-chat-sidebar-inner">
                <h3>Model Chat${context.project ? ` – ${context.project.name}` : ''}</h3>
    ...
    function renderMessage(role, content) {
        ...
        if (link) {
            link.addEventListener('click', () => {
                ...
                // Poll for the message element and scroll when found
                let attempts = 0;
                const maxAttempts = 20;
                const poll = () => {
                    const li = document.querySelector(`li[data-msg-id="${id}"]`);
                    if (li) {
                        console.log(`[Sidebar] Found li for msgId: ${id}, scrolling now.`);
                        window.scrollToMessage(id);
                    } else if (attempts < maxAttempts) {
                        attempts++;
                        setTimeout(poll, 100);
                    } else {
                        console.warn(`[Sidebar] Could not find message li for msgId: ${id} after polling.`);
                        window.scrollToMessage(id); // fallback
                    }
                };
                poll();
            });
